# `webapp/` — 실습 웹앱

교재 `pandas.md` 의 짝. 글로 전달되지 않는 것을 **실행으로** 보여 준다.
장 번호와 색 배정을 교재와 일치시킨다(교재 7장 = 웹앱 7장).

## 구조

```
webapp/
  API.md          ★ 모듈 작성 계약. 화면 모듈을 쓰기 전에 반드시 읽는다
  build.js        원본 → src/core/data.js 생성 + 단일 파일 인라이너
  src/
    theme.css  app.css
    core/
      df.js       미니 DataFrame 엔진 (메인이 직접 작성)
      ui.js       공용 위젯
      app.js      셸 — 챕터 등록소·라우터·진도·테마
      data.js     실습 데이터 (빌드 생성. 손으로 고치지 않는다)
    modules/
      chNN-*.js   1부 장별 화면. 파일명 사전순 = 표시 순서
      vN-*.js     2부(데이터 시각화) 장별 화면. part:2 — 셸이 (부, 번호) 로 정렬한다
      quest.js    스스로 하기(과제) 화면. 장이 아니다 — extra:true
  test/
```

## 절대 규칙 (전부 NumPy Lab 에서 실제로 깨져 본 것)

1. **로드 순서: `df → ui → data → app → modules → boot`.**
   `app.js` 가 `window.Lab` 을 정의하고 모듈이 로드 시점에 `Lab.register(...)` 를 부른다.
   순서가 뒤바뀌면 모듈이 **조용히 `Lab is not defined` 로 죽어** 장이 하나도 등록되지 않는다.
   문법 검사도 node 하네스도 통과한다. **브라우저로 열어 보지 않으면 절대 발견하지 못한다.**
2. **ES 모듈 문법(`import`/`export`) 금지.** 단일 파일 배포본에 인라인되므로 깨진다.
   모든 소스는 `window.X` 에 붙는 IIFE.
3. **`render(root)` 는 여러 번 호출된다.** 장을 다시 방문하면 다시 불린다.
   모듈 전역에 가변 상태를 두면 두 번째 방문에서 깨진다. `render` 안의 지역 변수 + `rebuild()` 패턴.
4. **화면의 숫자를 하드코딩하지 않는다.** `DF` 엔진으로 그 자리에서 계산한다.
   이것이 이 프로젝트의 존재 이유다.
5. **표 위젯의 HTML escape 는 열(`cols`) 속성이다.** `raw: true` 를 행에 붙이면 `<b>` 가 글자로 보인다.
6. **빌드는 원본이 없으면 기존 `data.js` 를 보존한다.** `buildData()` 가 `null` 을 반환하면
   "기존 파일 유지" 를 뜻한다. 그러지 않으면 저장소를 클론한 사람이 빌드하는 순간 데이터가 전부 날아간다.
7. **셸 구조는 NumPy Lab 과 한 벌이다.** https://yakimbss26.github.io/numpy-lab/ 과
   역할색·상태색이 같고 셸(사이드바·홈 화면·이전/다음·오른쪽 목차·모바일 서랍)도 같다.
   학생이 한 학기에 두 사이트를 함께 쓰므로 **같은 자리를 눌러야 한다.**
   **활자와 여백만 pandas 쪽이 앞서 있다**(Pretendard·넓은 여백). 아래 "디자인" 절 참고.

## 데이터 정책

배포본은 **합성 데이터**를 쓴다(타이타닉·라멘·전복). `lab_earthquake.csv` 만 USGS 퍼블릭
도메인이라 실데이터다. **합성인 데이터는 화면에 합성이라고 밝힌다.**
**2부의 네 데이터(서울 기온 2 · 부산 강수 · 시도 인구)는 실데이터다** — 기상청 공공누리 제1유형, 행안부 통계.
셸이 2부 장 머리에 출처 줄을 붙인다. `data.js` 에는 압축(K2 부호화)해서 싣는다 — `build.js` 의 `addPart2`.

### 화면의 코드는 "붙여 바로 실행된다" 고 약속한다 — 실제로 돌려서 확인한다

`UI.code(src, { dataset })` 의 복사 버튼은 머리말(import · 글꼴 · 읽기)을 붙여 준다. 2부를 만들 때
모든 복사 가능한 블록을 브라우저에서 모아 **진짜 pandas 로 돌려 봤더니** 이런 것이 나왔다:
정의되지 않은 변수(`연강수량`, `연도별월합계`), 인덱스를 안 바꾼 `pop.loc['제주특별자치도', …]`,
머리말이 열을 더해 화면과 달라진 `shape`, 학생에게만 뜨는 `vert` 경고, `print` 가 실제로 찍는 자릿수.
**화면 모듈을 고치면 이 검사를 다시 돌린다** — 방법은 `test/CLAUDE.md` 의 "화면 코드 실행 검사".
교재 본문은 원본 파일명을 그대로 쓰므로(학생이 자기 파일로 따라할 수 있게), 합성 이야기는
웹앱 화면과 README 에서만 한다. 자세한 근거는 `../CLAUDE.md` §10·§11.

## 색 배정 (전 장 고정. 검증기로 확정한 값)

| 역할 | 라이트 | 다크 | CSS 변수 |
|:---|:---|:---|:---|
| **원본** | `#2a78d6` | `#3987e5` | `--c-original` |
| **사본** | `#eb6834` | `#d95926` | `--c-copy` |
| **결과** | `#1baf7a` | `#199e70` | `--c-result` |
| **강조** | 색 아님 — 2px 링 + 굵은 라벨 | | `--c-focus-ring` |
| **에러/경고** | `#d03b3b` | `#d03b3b` | `--c-danger` |
| **결측(NaN)** | `#898781` + 45° 해칭 | 같음 | `--c-na`, `url(#na-hatch)` |

학생이 색으로 역할을 익히므로 **절대 바꾸지 않는다.**
**강조는 노랑이 아니다** — 노랑을 넣으면 주황과 구분되지 않아 검증기가 두 모드 모두 FAIL 을 냈다
(다크 CVD ΔE 4.8, 라이트 정상시야 ΔE 13.7). 근거는 `../CLAUDE.md` §7-1.

라이트 모드의 결과색은 표면 대비 2.74:1 이라 **숫자 라벨 또는 표 보기를 반드시 함께** 낸다.
계열이 4개 이상 필요하면 색을 추가하지 말고 "그 외" 로 접거나 차트를 나눈다.

차트를 만들기 전에 `dataviz` 스킬을 로드하고, 팔레트를 건드렸다면 `validate_palette.js` 를 다시 돌린다.

## 디자인

두 사이트를 한 학기에 함께 쓴다. **같은 것은 같아 보여야 하고 같은 자리에 있어야 한다.**
지금 상태는 **구조는 한 벌, 활자는 pandas 가 앞서 있음**이다.

| 무엇 | 어디에 | NumPy Lab 과 |
|:---|:---|:---|
| 역할색·상태색·순차 램프 | `src/theme.css` `:root` | **같다** (팔레트 검증기로 확정) |
| 표면·잉크·그림자 값 | `src/theme.css` `:root` | **같다** |
| 셸 배치(사이드바 268px · 모바일 서랍 900px) | `src/app.css` | **같다** |
| 셸 동작(홈 타일 · 진도 점 · 이전/다음 · 오른쪽 목차) | `src/core/app.js` | **같다** |
| 탭 아이콘(파란 사각형 + 모노 글자)·제목·설명 | `build.js` 의 `FAVICON`/`PAGE_TITLE`/`PAGE_DESC` | 형식이 같다 |
| 서체·자간·행간·여백·본문 폭 | `src/theme.css` 타이포 토큰, `src/app.css` | **pandas 만 다듬음** |

활자 쪽 값과 근거는 `../CLAUDE.md` §7-2 에 표로 있다. numpy-lab 에 그대로 옮기면
다시 완전히 한 벌이 된다 — 옮길 것은 타이포·여백 토큰뿐이고 셸은 손댈 필요가 없다.

역할 토큰(`--c-original` 등)이 이 프로젝트의 이름이고, NumPy Lab 에서 가져온 CSS 가 쓰는
`--s1` `--surface` `--ink` 같은 이름은 **theme.css 의 `:root` 안에서 별칭으로 묶여 있다.**
`var()` 는 쓰는 시점에 풀리므로 별칭을 한 번만 선언하면 다크 재선언을 그대로 따라간다.
**새 이름을 만들지 말고 그 표를 통해 가리켜라.**

- 셸 클래스: `.shell .sidebar .brand .nav .nav-group .side-foot .prog-bar .main .main-inner
  .crumb .chapter-nav .topbar .scrim .toc .hero .tiles .tile`
- 본문 유틸: `.h-chapter .h-sec .h-sub .lede .muted .small .nowrap`
- 위젯: `.card .panel-title .note(.note--why/tip/ver/danger) .seg .btn .quiz .q-choice
  .control-row .tbl .chip .viz .codeblock .out-label .ctl .pills/.pill .stat-row/.stat .modal`
- 과제 전용: `.quest .q-no .q-answer .q-verdict .q-meta .q-flip .q-note
  .quest-who .quest-prog .badge/.badge-row .cert-name .quest-return .tile.quest-cta`

**`.panel-title` 은 카드 소제목이다.** 오른쪽 목차가 이걸 긁어 가므로 **작은 딱지에 쓰지 마라** —
"출력" 같은 딱지에 쓰면 목차가 그 단어로 도배된다. 그런 딱지는 `.out-label` 을 쓴다.

`.control-row` 는 **컨트롤이 둘 이상이면** `:has()` 로 회색 판이 된다(NumPy Lab 의 `.controls`).
하나뿐이면 판이 되지 않는다 — 버튼 하나가 상자에 담기면 과장돼 보인다.
